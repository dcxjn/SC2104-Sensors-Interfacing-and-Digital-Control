...
void OLED_WR_Byte(uint8_t dat, uint8_t DataCmd)
{
	...
	{						 // Complete the code below
		OLED_SCLK_Clr();	 // set clk to low
		if (dat & 0x80)		 // read MSB
			OLED_SDIN_Set(); // set input
		else
			OLED_SDIN_Clr(); // clear input
		OLED_SCLK_Set();	 // set clk to high
		dat <<= 1;			 // shift to read next bit
	}
